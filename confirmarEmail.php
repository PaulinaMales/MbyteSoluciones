...
$tokenEmail = isset($_GET['token']) ? $_GET['token'] : null;
?>
<!DOCTYPE html>
<html lang="en">
				...
				<div class="row">
					<div class="col-md-12">
						<div class="newsletter">
							<p>CONFIRMACIÓN DE <strong>CORREO ELECTRÓNICO</strong></p>
						...
						<!-- Puedes agregar un elemento oculto para almacenar el ID de la categoría -->
							<input type="hidden" id="tokenEmail" value="<?php echo $tokenEmail; ?>">
								  <p class="shadowFrame"><svg version="1.1" class="shadow" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="61px" y="20px"
									 ...
									 xml:space="preserve">
								<ellipse fill="#1F4172" cx="61.128" cy="19.872" rx="49.25" ry="8.916"/>
									</svg></p>
								</div>							
							
								<div class="" type="email" >
									<strong>El registro de se cuenta fue exitoso. Bienvenido a MByte!</strong>
								</div>
								<br>
								<a class="primary-btn cta-btn" href="https://mbytesoluciones.com/login.php">Comenzar Ahora</a>	
								<br><br>
							</form>
						</div>
					</div>
...
$(document).ready(function () {
    var token = $('#tokenEmail').val();

    var api_confirmar = 'https://mbytesolucionesapi.onrender.com/api/confirmar/' + token;
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.msg === "Token confirmado, ya puedes iniciar sesión") {
                // Mostrar la vista deseada o realizar otras acciones según tus necesidades
                mostrarVista();
				
            } else {
                // Manejar el caso en el que la confirmación no fue exitosa
                console.error("La confirmación del token no fue exitosa. Mensaje:", data.msg);
                alert("La confirmación del token no fue exitosa. Mensaje: " + data.msg);
                window.location.href = "https://mbytesoluciones.com/login.php";
            }
        },
        error: function (error) {
            console.error("Error al confirmar el token", error);
            //alert("Error al confirmar el token");
    ...
    function mostrarVista() {
        window.location.href = "https://mbytesoluciones.com/confirmarEmail.php";
    }
});
