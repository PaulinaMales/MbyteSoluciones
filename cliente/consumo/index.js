            ...
            var html =
                '<div class="col-md-4 col-xs-6">' +
                    '<div class="product">' +
                        ...
                        '<button type="button" class="add-to-cart-btn" data-producto-id="' + producto._id + '" onclick="agregarProducto(\'' + producto._id + '\')"><i class="fa fa-shopping-cart"></i> Agregar al Carrito</button>'
                        '</div>' +
                    '</div>' +
                '</div>';
...
function mostrarMarcas(marcas) {
    ...
    $("#marcaBusqueda").append('<option value="0">-- Todos --</option>');

    // Iterar sobre las marcas y agregarlas al dropdown
    for (var i = 0; i < marcas.length; i++) {
        var marca = marcas[i].marca;
        $("#marcaBusqueda").append('<option value="' + marca + '">' + marca + '</option>');
    }
}
...
function obtenerProductosPorMarcaYCategoria(categoriaId) {
    ...
    var marcaSeleccionada = $('#marcaBusqueda').val();
